<?php include_once (APPPATH.'views/_layout/site/header_panel.php'); ?>






<div class="col-lg-9 col-md-9 col-sm-9 col-xs-12 left_panel f_l p0">

<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12  t_t_clip p0" >
            
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 niazmandi_ghavanin p0">
            
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 p0">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 sabad_kharid_info p0">
                <div class="f_r img_panel">
                    <img src="<?=base_url('')?>assets/site/img/logo.png" id="imgshow">
                    <div class="f_r img_panel_input">
                        <img src="<?=base_url('')?>assets/site/img/camera.svg">
            <input id="upload_img" class="col-lg-12 col-md-12 col-sm-12 col-xs-12 u_i c_btn workout_btn " type="file" multiple=""></div>
                </div>
                <div class="col-lg-10 col-md-9 col-sm-9 col-xs-12 f_l m_5 p0">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12  text_1_2 f_r title_info_big  p0">
                        کالای اصل را از ما بخرید
                    </div>
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 title_info_small p0">
                        <div class="f_r title_c">کد کاربری :</div>
                        <div class="f_r">
                            ۲۶۸۴۸۵
                        </div>
                    </div>
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 title_info_small f_r p0">
                    <div class="f_r title_c">آخرین به روز رسانی :</div>
                    <div class="f_r">۱۳۹۷/۰۸/۰۵</div>
                    </div>
                </div>
                
                
            </div>
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 sabad_div_info ">
                پنل کاربری / سبد خرید
        </div>

    </div>
</div>
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 niazmandi_ghavanin">
                
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 all ">
   
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 sabad_div_info ">
    پنل کاربری / سبد خرید
</div>
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 niazmandi_ghavanin p0">

<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 p0">
<div class="ghavanin_2 fnt_18">
آخرین فعالیت ها
<div class="bottom"></div>
</div>
<div class=" col-lg-12 col-md-12 col-sm-12 col-xs-12 ghavanin_2 line_h_2_5">
توضیحات کوتاه در مورد این بخش 
</div>
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 ghavanin_2 p0">

          <div   class="col-lg-12 col-md-12 col-sm-12 col-xs-12  p0">
              <ul class="p0">
                  <li class="col-lg-12 col-md-12 col-sm-12 hidden-xs today-li-tittle p_0_5 p0">
                      <ul class="p0">
                          <li class="col-lg-3 col-md-2 col-sm-3 col-xs-7 today-li-inner-tittle f_r p0">شناسه کاربری</li>
                          <li class="col-lg-3 col-md-3 col-sm-3 col-xs-2 today-li-inner black f_r p0">مشخصات کد</li>
                          <li class="col-lg-2 col-md-2 hidden-sm hidden-xs today-li-inner red f_r p0">ظرفیت استفاده</li>
                          <li class="col-lg-2  col-md-3 col-sm-3 col-xs-3 today-li-inner black f_r p0">استفاده شده</li>
                          <li class="col-lg-2 col-md-2 col-sm-3 hidden-xs today-li-inner green f_r  p0">تایید خرید</li>
                      </ul>
              </li>
                  <li class="col-lg-12 col-md-12 col-sm-12 col-xs-12 today-li m_2 p0">
                   <ul class=" p0">
                          <li  class="col-lg-3 col-md-2 col-sm-3 col-xs-12  today-li-inner-tittle f_r h_160 p0">
                       <div class="col-lg-6 col-md-12 col-sm-12 col-xs-12 pic_logo vertical_al f_r p0">
                              <img src="<?=base_url('')?>assets/site/img/logo.png" class="vertical_al">
                           <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 pos_rel p0">28598</div>
                              
                        </div>
                       <div class="col-lg-6 hidden-md hidden-sm hidden-xs f_l p0">
                              <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 li_inner_black p0"></div>
                              مجتبی بحیرایی
                              
                              </div>
                       
                       </li>
                          <li class="col-lg-3 col-md-3 hidden-sm col-xs-12  today-li-inner black f_r p0">
                              <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 li_inner_black p0">123456789</div>
                              <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 li_inner_black p0">تاریخ خرید</div>
                              <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 li_inner_black p0">۱۳۹۸/۰۴/۰۴</div>
                       </li>
                          <li class="col-lg-2 col-md-2 col-sm-3 col-xs-12  today-li-inner m_20_noxs line_h_1_5 f_r red f_r p0">
                              <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 li_inner_black p0"></div>
                              ۲۰</li>
                          <li class="col-lg-2 col-md-3 col-sm-3 col-xs-12  today-li-inner  f_r green f_r p0">
                              <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 li_inner_black p0"></div>
                            <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 f_r p0">
                                <button class="f_l p0" id="inc" onclick="btn_inc()">
                                    <img src="<?=base_url('')?>assets/site/img/plus_perp.svg"></button>
                              </div>
                            <div id="number_inc_dec" class="col-lg-4 col-md-4 col-sm-4 col-xs-4 f_r today-li-inner  line_h_4 p0">
                                10
                              </div>
                            <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 f_l p0">
                                <button class="f_r p0" id="dec" onclick="btn_dec()">
                                    <img src="<?=base_url('')?>assets/site/img/minus_perp.svg"></button>
                              </div></li>
                          <li class="col-lg-2 col-md-2 col-sm-3 col-xs-12  today-li-inner line_h_1_5 f_r black f_r  p0">
                              <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 li_inner_black p0"></div>
                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 f_r p0">
                                <button class="f_l p0" >
                                <img src="<?=base_url('')?>assets/site/img/accept.svg" ></button>
                              </div>
                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 f_l p0">
                                <button class="f_r p0" >
                                <img src="<?=base_url('')?>assets/site/img/denied.svg" ></button>
                              </div>
                       </li>
                      </ul>
                  </li>
                  
                  </ul>
          </div>
                    
</div>

</div>


</div>
</div>

                
                </div>
                
            </div>
                
                </div>
                
            </div>








<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 clear_both"></div>



    
  
<?php include_once (APPPATH.'views/_layout/site/footer_panel.php'); ?>